...
export default function App() {
  ...
  const LoadingOverlay = () => (
    <div className="fixed inset-0 z-[100] bg-white/80 dark:bg-black/90 backdrop-blur-3xl flex flex-col items-center justify-center animate-fade-in-up duration-500">
      ...
      <h2 className="mt-8 text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-400 animate-pulse">
  ...
  const renderContent = () => {
    switch (viewState.currentView) {
      case 'landing':
        return (
          <div key="landing" className="animate-fade-in-up flex flex-col items-center justify-center py-16 sm:py-24 text-center hardware-accelerated">
            <div className="mb-10 relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-brand-400 to-purple-600 rounded-full blur opacity-40 group-hover:opacity-75 transition duration-1000 animate-pulse"></div>
              <div className="relative bg-white dark:bg-black rounded-full p-6 ring-1 ring-gray-900/5 shadow-2xl hover:scale-105 transition-transform duration-500 ease-spring">
                 <BookOpen className="w-16 h-16 text-brand-600 dark:text-brand-400" />
              </div>
            </div>
            
            <h1 className="text-4xl sm:text-7xl font-extrabold text-gray-900 dark:text-white mb-6 tracking-tight leading-tight drop-shadow-sm">
            ...
            <p className="max-w-2xl text-lg sm:text-xl text-gray-600 dark:text-gray-300 mb-12 leading-relaxed font-medium">
            ...
            <div className="flex flex-col sm:flex-row gap-5 w-full sm:w-auto items-center justify-center">
              <button 
                ...
                className="group relative px-8 py-4 bg-brand-600 hover:bg-brand-700 text-white rounded-2xl font-bold text-lg shadow-xl shadow-brand-500/30 transition-all hover:scale-105 active:scale-95 ease-spring w-full sm:w-auto overflow-hidden"
              >
            ...
            <div className="mt-20 grid grid-cols-1 sm:grid-cols-3 gap-8 w-full max-w-5xl text-left">
              {[
                ...
                <div key={i} className="glass-card p-8 rounded-[2rem] hover:scale-[1.03] transition-transform duration-400 ease-spring">
                  <item.icon className="w-10 h-10 text-brand-500 mb-5" />
      ...
      case 'class-select':
        return (
          <div key="class-select" className="animate-fade-in-up">
            <h2 className="text-4xl font-bold text-center text-gray-900 dark:text-white mb-16">Select Your Class</h2>
      ...
      case 'subject-select':
        return (
          <div key="subject-select" className="animate-fade-in-up">
            <button onClick={() => setViewState(prev => ({...prev, currentView: 'class-select'}))} className="mb-8 flex items-center text-gray-600 dark:text-gray-400 hover:text-brand-600 dark:hover:text-brand-400 font-medium transition-colors hover:scale-105 active:scale-95 origin-left px-4 py-2 bg-white/20 rounded-lg backdrop-blur-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {viewState.selectedClass && SUBJECTS_BY_CLASS[viewState.selectedClass].map((subject, idx) => (
                <button
                  ...
                  className="glass-card p-8 rounded-[2rem] text-left hover:scale-[1.03] active:scale-[0.98] transition-all duration-400 ease-spring group animate-fade-in-up"
                >
      ...
      case 'topic-select':
        return (
          <div key="topic-select" className="animate-fade-in-up max-w-3xl mx-auto">
            <button onClick={() => setViewState(prev => ({...prev, currentView: 'subject-select'}))} className="mb-8 flex items-center text-gray-600 dark:text-gray-400 hover:text-brand-600 dark:hover:text-brand-400 font-medium transition-colors hover:scale-105 active:scale-95 origin-left px-4 py-2 bg-white/20 rounded-lg backdrop-blur-sm">
            ...
            {/* Custom Topic Input */}
            <div className="glass-card p-8 rounded-[2rem] mb-12 transform hover:scale-[1.01] transition-transform duration-300">
              ...
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 px-6 py-4 rounded-xl bg-gray-50/50 dark:bg-black/50 border border-gray-200 dark:border-gray-700 focus:ring-4 focus:ring-brand-500/20 focus:border-brand-500 focus:outline-none dark:text-white transition-all text-lg"
                ...
                <button
                  ...
                  className="px-8 py-4 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white rounded-xl font-bold transition-all hover:scale-105 active:scale-95 shadow-lg shadow-brand-500/20"
                >
            ...
            {viewState.isLoadingChapters ? (
              <div className="flex flex-col items-center justify-center py-16 bg-white/20 dark:bg-black/20 rounded-3xl backdrop-blur-sm">
                 <Loader2 className="w-12 h-12 text-brand-500 animate-spin mb-6" />
                 <p className="text-gray-500 dark:text-gray-400 animate-pulse font-medium text-lg">Curating syllabus...</p>
              </div>
            ) : (
              <div className="space-y-10">
                {viewState.availableChapters?.map((cat, idx) => (
                  <div key={idx} className="animate-fade-in-up" style={{ animationDelay: `${idx * 150}ms` }}>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-5 px-4 border-l-4 border-brand-500">
                      {cat.category}
                    </h3>
                    <div className="grid gap-4">
                      {cat.chapters.map((chapter, cIdx) => (
                        <button
                          ...
                          className="w-full text-left p-5 glass-card rounded-2xl hover:bg-brand-50/80 dark:hover:bg-brand-900/30 transition-all hover:scale-[1.01] active:scale-[0.99] flex justify-between items-center group border border-transparent hover:border-brand-200 dark:hover:border-brand-800"
                        >
                          <span className="font-semibold text-lg text-gray-700 dark:text-gray-200 group-hover:text-brand-700 dark:group-hover:text-brand-300 transition-colors">
                            {chapter}
                          </span>
                          <ChevronRight className="w-5 h-5 text-gray-400 group-hover:text-brand-500 opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-0 -translate-x-3" />
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            {error && (
               <div className="mt-8 p-6 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-2xl text-center font-medium animate-pop-in border border-red-100 dark:border-red-800 shadow-sm">
                 {error}
               </div>
            )}
          </div>
      ...
      case 'ai-tutor':
        return (
          <div key="ai-tutor" className="animate-scale-in">
            <AITutor 
      ...
      case 'syllabus-analysis':
        return (
          <div key="syllabus-analysis" className="animate-fade-in-up">
      ...
      case 'bookmarks':
        return user ? (
          <div key="bookmarks" className="animate-fade-in-up">
            <BookmarksView 
              user={user} 
